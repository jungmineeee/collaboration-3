...
class GUI(QMainWindow):
    ...
    #     pixel_x = int((self.map_x + odom_x) / self.resolution)
    #     pixel_y = int(self.height - ((self.map_y + odom_y) / self.resolution))
    ...
    def input_image(self):
        ...
        try:
            ...
            pixel_x = int((self.amcl_pose_x - (-2.0)) / self.resolution)  # 원점 기준 x 변환
            pixel_y = int((self.amcl_pose_y - (-17.0)) / self.resolution)  # 원점 기준 y 변환

            # 회전된 이미지 좌표계 보정
            corrected_pixel_x = pixel_y  # y좌표 -> x좌표
            corrected_pixel_y = rotated_image.shape[0] - pixel_x  # x좌표 -> y좌표로 변환

            # 경계값 확인
            if (0 <= corrected_pixel_x < rotated_image.shape[1]) and (0 <= corrected_pixel_y < rotated_image.shape[0]):
                print(f"Updated robot position on rotated map: ({corrected_pixel_x}, {corrected_pixel_y})")
                # 빨간색 점 그리기
                cv2.circle(rotated_image, (corrected_pixel_x, corrected_pixel_y), 5, (0, 0, 255), -1)
            else:
